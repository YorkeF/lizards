...
function getPlant(PDO $pdo, int $id): array|false {
    $s = $pdo->prepare('SELECT * FROM plants WHERE id = ?');
    $s->execute([$id]);
    return $s->fetch(PDO::FETCH_ASSOC);
}

function getPlantImages(PDO $pdo, int $id): array {
    $s = $pdo->prepare('SELECT * FROM plant_images WHERE plant_id = ? ORDER BY sort_order, id');
    $s->execute([$id]);
...
if ($action === 'delete_image' && $id) {
    ...
    if ($imageId) {
        $s = $pdo->prepare('SELECT filename FROM plant_images WHERE id = ? AND plant_id = ?');
        $s->execute([$imageId, $id]);
        $img = $s->fetch(PDO::FETCH_ASSOC);
        if ($img) {
            $file = __DIR__ . '/../images/' . $img['filename'];
            if (file_exists($file)) unlink($file);
            $pdo->prepare('DELETE FROM plant_images WHERE id = ?')->execute([$imageId]);
        }
    }
    header("Location: edit-plant.php?id=$id");
    exit;
}
...
if ($action === 'upload_image' && $id && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $s = $pdo->prepare('SELECT COALESCE(MAX(sort_order), -1) FROM plant_images WHERE plant_id = ?');
    $s->execute([$id]);
    $sortOrder = (int)$s->fetchColumn() + 1;

    $insert = $pdo->prepare('INSERT INTO plant_images (plant_id, filename, sort_order) VALUES (?, ?, ?)');
    for ($i = 0; $i < $count; $i++) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action !== 'upload_image') {
    ...
    if ($fields['name'] === '') {
        ...
        if ($id) {
            ...
            $stmt = $pdo->prepare("UPDATE plants SET $set WHERE id = ?");
            $stmt->execute([...array_values($fields), $id]);
        } else {
            $cols         = implode(', ', array_keys($fields));
            $placeholders = implode(', ', array_fill(0, count($fields), '?'));
            $stmt         = $pdo->prepare("INSERT INTO plants ($cols) VALUES ($placeholders)");
...
$item = $id ? getPlant($pdo, $id) : null;
if ($id && !$item) { header('Location: plants.php'); exit; }
$images = $id ? getPlantImages($pdo, $id) : [];
$v      = $item ?? array_fill_keys(['name','scientific_name','description','price','available','obtained_from'], '');
$title  = $id ? 'Edit Plant' : 'Add Plant';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="image-grid">
      <?php foreach ($images as $img): ?>
        <div class="image-item">
          ...
          <a href="edit-plant.php?id=<?= $id ?>&action=delete_image&image_id=<?= $img['id'] ?>"
             class="delete-btn" onclick="return confirm('Remove this photo?')">✕</a>
        </div>
      <?php endforeach; ?>
    </div>
  <?php endif; ?>
  <form method="post" enctype="multipart/form-data" action="edit-plant.php?id=<?= $id ?>&action=upload_image">
...
<?php else: ?>
<div class="card"><p class="note">Save the plant first, then you can add photos.</p></div>
<?php endif; ?>
